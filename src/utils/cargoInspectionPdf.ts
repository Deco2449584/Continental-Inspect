...
/** Maps photo sources to inline &lt;img&gt; tags for reliable PDF rendering. */
export function buildVisualPhotoEvidenceHtml(photoSources: readonly string[]): string {
    ...
    .map((url, index) => {
      ...
      return `<div class="video-card">
        ...
        <div class="video-card-body">
          <a href="${escapeAttr(url)}" class="video-stream-btn">👉 Click here to stream Video Evidence</a>
...
async function resolveImageSourceForPdf(url: string, index: number): Promise<string> {
  ...
  try {
    const localPath = `${cacheDir}pdf_photo_${Date.now()}_${index}.jpg`;
...
async function resolvePhotoSourcesForPdf(urls: readonly string[]): Promise<string[]> {
  return Promise.all(urls.map((url, index) => resolveImageSourceForPdf(url, index)));
}

function buildInspectionHtml(inspection: CargoInspection, photoSources: readonly string[]): string {
...
<body>
  <div class="header">
    ...
    <div class="header-sub">Cargo inspection report · ${escapeHtml(brand.name)}</div>
  </div>
  ...
  <div class="section">
    ...
    <p class="muted" style="margin-top:0;margin-bottom:16px;">Click any link below to open the original file in Firebase Storage.</p>
  ...
  <div class="footer">
    <div>${escapeHtml(brand.appName)} · ${escapeHtml(brand.panelTitle)}</div>
...
export async function shareCargoInspectionPdf(inspection: CargoInspection): Promise<void> {
  const photoUrls = inspection.photoEvidence ?? [];
  const photoSources =
    photoUrls.length > 0 ? await resolvePhotoSourcesForPdf(photoUrls) : [];

  const { uri } = await Print.printToFileAsync({
    html: buildInspectionHtml(inspection, photoSources),
  });
